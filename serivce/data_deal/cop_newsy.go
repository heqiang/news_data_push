...
func CopyNews(sourceNewsPath, dstPath string, config *config.ServerConfig) {
	...
	if len(fileList) > 0 {
		...
		for _, dirPath := range fileList {
			...
			if filetools.IsFileExist(currDatpath) {
				for _, path := range filetools.ListDir(currDatpath) {
...
func moveImageOrFile(srcPath, dstPath string) int {
	copyNum := 0
	for _, k := range filetools.ListDir(srcPath) {
